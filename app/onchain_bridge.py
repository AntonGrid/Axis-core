...
class OnchainAttestationParams:
    attestation_id: bytes   # 32 байта, для bytes32
    device_id: bytes        # 32 байта, для bytes32
    allowed: bool
...
def _to_bytes32_hash(value: str) -> bytes:
    """
    keccak256 от UTF-8 строки -> 32-байтовый хеш (для bytes32).
    """
...
def _parse_issued_at(issued_at: str) -> int:
    """
    Преобразует ISO 8601 с 'Z' в конце в unix timestamp (секунды).
    Пример: '2026-07-25T19:05:00Z' -> 1780001100
    """
...
def build_attestation_params(attestation: Dict[str, Any]) -> OnchainAttestationParams:
    """
    Строит параметры для вызова submitAttestation(...) из JSON-аттестации Oracle.
