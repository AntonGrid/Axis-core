"""EVM domain adapter: attestation -> on-chain parameter mapping.

This module is specific to Ethereum-compatible ledgers (keccak256, bytes32,
``submitAttestation``-style calldata). It lives in ``adapters/`` so the core
package stays blockchain-agnostic (ADR: protocol vs implementation).
"""
